...
For $P(X\le 4), \lambda=4.5$,

\begin{align*}
p_0 &= 0.9995^{9000} \\ 
p_1 &= \frac{1}{1 \cdot 0.9995} (9000 - 1 + 1) \cdot 0.0005 \cdot p_0 \\ 
p_2 &= \frac{1}{2 \cdot 0.9995} (9000 - 2 + 1) \cdot 0.0005 \cdot p_1 \\ 
p_3 &= \frac{1}{3 \cdot 0.9995} (9000 - 3 + 1) \cdot 0.0005 \cdot p_2 \\
p_4 &= \frac{1}{4 \cdot 0.9995} (9000 - 4 + 1) \cdot 0.0005 \cdot p_3 \\
P(X\le 4) &= \sum_{i=0}^{4} p_{k} \\ 
&= 0.5321
.\end{align*}
\end{solution}
...
\begin{align*}
p_0 &= \exp(-4.5) \\ 
p_1 &= -\frac{4.5}{1} \cdot p_0 \\ 
p_2 &= -\frac{4.5}{2} \cdot p_1 \\
p_3 &= -\frac{4.5}{3} \cdot p_2 \\
p_4 &= -\frac{4.5}{4} \cdot p_3 \\
P(X\le 4) &= \sum_{i=0}^{4} p_{k} \\ 
&= 0.5321
.\end{align*}

The recursive formula of the Poisson distribution shows that it can be used to
estimate the binomial distribution for larges values of $n$ and small values of
$p$ since in the binomial distribution formula, $p^{n} \approx e^{-np} =
e^{-\lambda}$ if $n$ is large and $p$ is small (can verify using Taylor
expansion of $e^{-np}$ and $p^{n} = e^{n\ln p}$ and $\ln p \to 1$ as $p \to 0$),
$(kq)^{-1} \to \frac{1}{k}$  as $p \to 0 \iff q \to 1$, and the $(n - k + 1)
\cdot p \to np = \lambda$ if $n \gg k$.
\end{solution}
\end{parts}
...
\implies \tan^{-1}(x) = 0.4\pi \implies x = \tan(0.4\pi) = 3.078
.\] 
\end{solution}
\end{parts}
...
\begin{solution}
\[
P(x \le k) = 1-q^{k+1} = 1 - 0.5^{k+1} = 0.99 \implies k = 5.64
.\] 
\end{solution}
...
Thus, the probability of receiving at least one message in 10 minutes is
